    ...
    <div id="product-card" class="card shadow-lg p-4 rounded-lg text-center bg-light border-success">
        ...
        <p class="fs-5"><strong>Categoria:</strong> <span class="text-dark">{{ $prodotto->categoria->categoria->nome_categoria}}</span></p>
...
<script>
    ...
    document.getElementById('save-btn').addEventListener('click', function() {
        ...
        let productId = "{{ $prodotto->id_prodotto }}";

        fetch(`/fridge_dashboard`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
            body: JSON.stringify({ id_prodotto: productId, nome_prodotto: name, data_scadenza: expiry })
